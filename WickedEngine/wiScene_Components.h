...
namespace wi::scene
{
	...
	struct alignas(16) FlammableComponent
	{
		enum FLAGS
		{
			NONE = 0,
		...
		float flammability = 0.6f; // [0,1+] multiplier applied to heat transfer and ignition
		float fuel = 60.0f; // total virtual fuel amount (seconds * burnRate)
		float initialFuel = 60.0f; // stored to scale effects
		float burnRate = 10.0f; // fuel consumption per second while burning
		float ignition = 30.0f; // heat threshold to ignite
		float heat = 0.0f; // accumulated heat
		float heatDissipation = 5.0f; // rate at which heat is lost when not burning
		float spreadRadius = 4.0f; // meters
		float spreadRate = 15.0f; // heat transfer per second to neighbours
		float windSensitivity = 1.0f; // multiplier for wind influence
		float smokeAmount = 1.0f; // scales smoke spawn rate
		float scorchFactor = 0.35f; // darkening factor once burnt
		uint32_t scorchApplied = 0;

		wi::ecs::Entity fireEmitter = wi::ecs::INVALID_ENTITY;
		wi::ecs::Entity smokeEmitter = wi::ecs::INVALID_ENTITY;
		wi::ecs::Entity distortionEmitter = wi::ecs::INVALID_ENTITY;
		wi::ecs::Entity sparksEmitter = wi::ecs::INVALID_ENTITY;
		float damage = 0.0f; // accumulated damage applied to the host object
		float cooldown = 0.0f; // cooldown timer when extinguished
		float timeSinceIgnition = 0.0f;

		// Non-serialized runtime attributes:
		float incomingHeat = 0.0f;
		float intensity = 0.0f; // normalized [0,1] current fire intensity

		constexpr bool IsActive() const { return _flags & ACTIVE; }
		constexpr void SetActive(bool value = true) { if (value) { _flags |= ACTIVE; } else { _flags &= ~ACTIVE; } }
		constexpr bool IsBurning() const { return _flags & BURNING; }
		constexpr void SetBurning(bool value = true) { if (value) { _flags |= BURNING; _flags &= ~BURNT; } else { _flags &= ~BURNING; } }
		constexpr bool IsBurnedOut() const { return _flags & BURNT; }
		constexpr void SetBurnedOut(bool value = true) { if (value) { _flags |= BURNT; _flags &= ~BURNING; } else { _flags &= ~BURNT; } }
		constexpr bool IsAutoGenerated() const { return _flags & AUTO_GENERATED; }
		constexpr void SetAutoGenerated(bool value = true) { if (value) { _flags |= AUTO_GENERATED; } else { _flags &= ~AUTO_GENERATED; } }
