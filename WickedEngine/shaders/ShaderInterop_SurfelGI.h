...
static const float SURFEL_COVERAGE_MIN_WEIGHT = 0.01f; // flicker-safe gather cost cut: in the per-pixel GI gather, skip the expensive moment sample + SH eval for a contributor whose loose weight (radial * dotN) is below this - such a surfel barely moves the normalized average. This is a PER-SURFEL, order-independent test, NOT an accumulation-threshold early-out (that would average a different binning-order-dependent subset each frame and shimmer). Seen-marking, the coverage metric and the spacing nearest are still done for skipped contributors, so the recycler and spawner are unaffected. Higher = skip more = cheaper, at the cost of a slightly larger but STABLE (non-flickering) bias toward near/high-weight surfels; 0 disables the skip
