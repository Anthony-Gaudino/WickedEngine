...
VertextoPixel main(uint vertexID : SV_VertexID, uint instanceID : SV_InstanceID)
{
	...
	if (image.IsFont())
	{
		...
		// Carried for the same reason the image branch carries it: the pixel
		// shader reconstructs a world position from it to clip and fog a glyph
		// that stands in the scene, and SV_Position has already been divided
		// through by the time it arrives.
		Out.screen = Out.pos;
	}
	else
	{
