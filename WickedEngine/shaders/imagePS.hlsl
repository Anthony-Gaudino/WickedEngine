...
float4 main(VertextoPixel input) : SV_TARGET
{
	...
	// that lives in the world - a sprite or, since the font renderer shares
	// this shader, a glyph. Both tests share one world position, and both come
	...
	// The water between this draw and the eye. Flagged rather than inferred,
	...
	//
	// Outside the branch above, so that a glyph standing in the scene is fogged
	// by the water it is seen through exactly as a sprite is.
	[branch]
	if (image.flags & IMAGE_FLAG_UNDERWATER_FOG)
	{
		...
		if (image.IsFont())
		{
			// A glyph is drawn premultiplied, so the veiling light has to be
			// scaled by the coverage too - otherwise the antialiased edge of a
			// character paints haze where it barely covers.
			ApplyWaterFogPremultiplied(screenUV, P, fogged);
			ApplyVolumetricLightPremultiplied(screenUV, P, fogged);
		}
		else if (image.flags & IMAGE_FLAG_UNDERWATER_FOG_ADDITIVE)
		{
